...
export default function ResidenteTable({ moduleColor }) {
  ...
  const handleEliminar = async (id) => {
    try { await deleteResidente(id); setConfirmId(null); fetchData() }
    ...
    <>
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <button className="btn btn-sm btn-primary" onClick={() => { setSelected(null); setShowModal(true) }}>
          <i className="bi bi-plus-lg me-1" />Nuevo Residente
        </button>
      </div>
      <div className="cms-table-wrap">
        <table className="table table-hover cms-table">
          <thead><tr><th>#</th><th>ID Persona</th><th>ID Propiedad</th><th>Tipo</th><th>F. Ingreso</th><th>F. Salida</th><th>Estado</th><th>Acciones</th></tr></thead>
          <tbody>
            {rows.map((row, i) => (
              <tr key={row.id ?? i}>
                ...
                <td><span className="badge text-bg-light border" style={{ fontSize: 10 }}>{row.tipoResidente}</span></td>
      ...
      <ResidenteModal show={showModal} residente={selected}
