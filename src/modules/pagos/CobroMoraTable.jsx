    ...
    <>
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <button className="btn btn-sm btn-primary" onClick={() => { setSelected(null); setShowModal(true) }}>
          <i className="bi bi-plus-lg me-1" />Nuevo Cobro Mora
        </button>
      </div>
      <div className="cms-table-wrap">
        <table className="table table-hover cms-table">
          <thead><tr><th>#</th><th>Cuenta</th><th>F. Cálculo</th><th>Días Atraso</th><th>Saldo Base</th><th>% Aplicado</th><th>Monto Mora</th><th>Acumulado</th><th>Acciones</th></tr></thead>
          <tbody>
            {rows.map((row, i) => (
              <tr key={row.id ?? i}>
                ...
                <td>Q {Number(row.saldoBase).toFixed(2)}</td>
                <td>{row.porcentajeAplicado}%</td>
                <td className="text-danger fw-semibold">Q {Number(row.montoMora).toFixed(2)}</td>
                <td className="fw-semibold">Q {Number(row.acumuladoTotal).toFixed(2)}</td>
                <td>
      ...
      <CobroMoraModal show={showModal} cobro={selected}
