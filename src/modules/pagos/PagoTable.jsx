...
const ESTADO_COLOR = { PENDIENTE: 'warning', APLICADO: 'success', ANULADO: 'danger', EN_REVISION: 'info' }
    ...
    <>
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <button className="btn btn-sm btn-primary" onClick={() => { setSelected(null); setShowModal(true) }}>
          <i className="bi bi-plus-lg me-1" />Nuevo Pago
        </button>
      </div>
      <div className="cms-table-wrap">
        <table className="table table-hover cms-table">
          <thead><tr><th>#</th><th>Factura</th><th>No. Recibo</th><th>Fecha Pago</th><th>Monto</th><th>Estado</th><th>Acciones</th></tr></thead>
          <tbody>
            {rows.map((row, i) => (
              <tr key={row.id ?? i}>
      ...
      <PagoModal show={showModal} pago={selected}
