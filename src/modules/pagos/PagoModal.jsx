...
export default function PagoModal({ show, onClose, onSaved, pago }) {
  ...
  useEffect(() => {
    if (pago) {
      // El backend devuelve camelCase: idPago, idFactura, etc.
      ...
      setIdMoneda(pago.idMoneda     ?? '')
      setTipoCambio(pago.tipoCambio ?? '')
      setMontoGtq(pago.montoEnGtq   ?? '')
      setMetodo(pago.idMetodoPago   ?? '')
      setBancoOr(pago.idBancoOrigen ?? '')
      setBancoDest(pago.idBancoDestino ?? '')
  ...
  const handleSubmit = async () => {
      ...
      try {
        const payload = {
          IdPago:           pago ? (pago.idPago ?? pago.IdPago ?? 0) : 0,
        ...
        if (pago) {
          await updatePago(payload)          // ← sin id en URL, todo en body
        } else {
