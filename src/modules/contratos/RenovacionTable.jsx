    ...
    <>
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <button className="btn btn-sm btn-primary" onClick={() => { setSelected(null); setShowModal(true) }}>
          <i className="bi bi-plus-lg me-1" />Nueva Renovación
        </button>
      </div>
      <div className="cms-table-wrap">
        <table className="table table-hover cms-table">
          <thead><tr><th>#</th><th>ID Contrato</th><th>Nueva Vigencia</th><th>Nuevo Monto</th><th>F. Registro</th><th>Acciones</th></tr></thead>
          <tbody>
            {rows.map((row, i) => (
              <tr key={row.id ?? i}>
      ...
      <RenovacionModal show={showModal} renovacion={selected}
