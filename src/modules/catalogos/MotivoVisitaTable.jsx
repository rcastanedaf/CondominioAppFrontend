...
import { getMotivosVisita, deleteMotivoVisita } from './motivoVisitaService'
import MotivoVisitaModal from './MotivoVisitaModal'
import { usePaginacion }  from '../../shared/hooks/usePaginacion'
import PaginacionFooter   from '../../shared/components/PaginacionFooter'

export default function MotivoVisitaTable({ moduleColor }) {
  ...
  const handleEliminar = async (id) => {
    try { await deleteMotivoVisita(id); setConfirmId(null); fetchData() }
    ...
    <>
      {/* Header + buscador + selector — reemplaza el antiguo d-flex justify-content-between */}
            <PaginacionFooter
              ...
              icono="bi-bank"
              labelBoton="Nuevo Motivo de Visita"
              onNuevo={() => { setSelected(null); setShowModal(true) }}
              moduleColor={moduleColor}
              filtro={filtro}
              setFiltro={setFiltro}
              placeholder="Filtrar motivo de visita..."
      ...
      {/* Footer paginación — segundo uso del mismo componente */}
            <PaginacionFooter
              ...
              moduleColor={moduleColor}
            />
      <MotivoVisitaModal show={showModal} motivoVisita={selected} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); fetchData() }} />
    </>
  )
}