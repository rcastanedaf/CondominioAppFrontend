...
export default function MetodoPagoTable({ moduleColor }) {
  ...
  const handleEliminar = async (id) => {
    try { await deleteMetodoPago(id); setConfirmId(null); fetchData() }
    ...
    <>
      {/* Header + buscador + selector — reemplaza el antiguo d-flex justify-content-between */}
            <PaginacionFooter
              titulo="Metodo Pago"
              icono="bi-bank"
              labelBoton="Nuevo Metodo Pago"
              onNuevo={() => { setSelected(null); setShowModal(true) }}
              moduleColor={moduleColor}
              filtro={filtro}
              setFiltro={setFiltro}
              placeholder="Filtrar metodo pago..."
      ...
      <MetodoPagoModal show={showModal} metodoPago={selected} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); fetchData() }} />
    </>
  )
}