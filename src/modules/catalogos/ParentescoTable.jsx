    ...
    <>
      {/* Header + buscador + selector — reemplaza el antiguo d-flex justify-content-between */}
            <PaginacionFooter
              titulo="Parentesco"
              icono="bi-bank"
              labelBoton="Nuevo Parentesco"
              onNuevo={() => { setSelected(null); setShowModal(true) }}
              moduleColor={moduleColor}
              filtro={filtro}
              setFiltro={setFiltro}
              placeholder="Filtrar parentesco..."
      ...
      {/* Footer paginación — segundo uso del mismo componente */}
            <PaginacionFooter
              ...
              moduleColor={moduleColor}
            />
      <ParentescoModal show={showModal} parentesco={selected} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); fetchData() }} />
    </>
  )
}