...
import { getTiposContrato, deleteTipoContrato } from './tipoContratoService'
import TipoContratoModal from './TipoContratoModal'
import { usePaginacion }  from '../../shared/hooks/usePaginacion'
import PaginacionFooter   from '../../shared/components/PaginacionFooter'

export default function TipoContratoTable({ moduleColor }) {
  ...
  const handleEliminar = async (id) => {
    try { await deleteTipoContrato(id); setConfirmId(null); fetchData() }
    ...
    <>
      {/* Header + buscador + selector — reemplaza el antiguo d-flex justify-content-between */}
            <PaginacionFooter
              ...
              icono="bi-bank"
              labelBoton="Nuevo Tipo Contrato"
      ...
      {/* Footer paginación — segundo uso del mismo componente */}
            <PaginacionFooter
              ...
              moduleColor={moduleColor}
            />
      <TipoContratoModal show={showModal} tipoContrato={selected} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); fetchData() }} />
    </>
  )
}
