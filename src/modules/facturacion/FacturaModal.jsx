...
export default function FacturaModal({ show, onClose, onSaved, factura }) {
  ...
  // ── Poblar al abrir ───────────────────────────────────────
  // Depende solo de `show` para garantizar reset/poblar cada vez que abre
  useEffect(() => {
    if (!show) return
    if (factura) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        // ✅ Usar idFactura (no id) para edición
        idFactura:             factura ? (factura.idFactura ?? factura.id) : 0,
      ...
      // ✅ Llamadas al backend descomentadas y corregidas
      if (factura) {
        await updateFactura(payload)
      } else {
